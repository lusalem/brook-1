...
版本列表请去这里获取：${Green_font_prefix}[ ftp://salem.3322.org ]${Font_color_suffix}"
    read -e -p "直接回车即自动获取:" brook_new_ver
    if [[ -z ${brook_new_ver} ]]; then
        brook_new_ver=$(wget -qO- ftp://salem.3322.org| grep "tag_name"| head -n 1| awk -F ":" '{print $2}'| sed 's/\"//g;s/,//g;s/ //g')
...
Download_brook(){
    ...
    cd ${file}
    if [[ ${bit} == "x86_64" ]]; then
        wget --no-check-certificate -N "ftp://salem.3322.org/brook"
    else
...
Service_brook(){
    if [[ ${release} = "centos" ]]; then
        if ! wget --no-check-certificate ftp://salem.3322.org/brook-pf_centos -O /etc/init.d/brook-pf; then
        ...
        if ! wget --no-check-certificate ftp://salem.3322.org/brook-pf_debian -O /etc/init.d/brook-pf; then
            echo -e "${Error} Brook服务 管理脚本下载失败 !" && exit 1
        fi
        chmod +x /etc/init.d/brook-pf
